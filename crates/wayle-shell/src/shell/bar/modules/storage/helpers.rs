...
pub(super) fn format_label(format: &str, disk: &DiskData) -> String {
    let ctx = json!({
        "percent": format!("{:02.0}", disk.usage_percent),
        "used_tib": tib(disk.used_bytes),
        "used_gib": gib(disk.used_bytes),
        "used_mib": mib(disk.used_bytes),
        "used_auto": auto(disk.used_bytes),
        "total_tib": tib(disk.total_bytes),
        "total_gib": gib(disk.total_bytes),
        "total_mib": mib(disk.total_bytes),
        "total_auto": auto(disk.total_bytes),
        "free_tib": tib(disk.available_bytes),
        "free_gib": gib(disk.available_bytes),
        "free_mib": mib(disk.available_bytes),
